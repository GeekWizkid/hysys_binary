...
class FunctionDataExtractor:
    ...
    def GetFunctionsData(self):
        ...
        apply_name_filter = root_name in TARGETS
        print(apply_name_filter)
        functionsData = PdbGeneratorPy.FunctionsData()
        for functionEa in Functions():
            function_name = ida_funcs.get_func_name(functionEa)
            if function_name.startswith(("unknown_libname_", "@__GetPolymorphicDTC$qpvui_", "@std@", "__GetExceptDLLinfoInternal_", "_", "@",)):
                continue
            if apply_name_filter and function_name.startswith("sub_"):
                continue;
    ...
    def GetFunctionData(self, FunctionEa, ExecuteSync):
        ...
        def GetFunctionDataInternal():
            ...
            #print(functionName) - раскоментировать если проблемы со структурами (решетка в имени)

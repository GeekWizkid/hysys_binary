...
class TypeExtractor:
    ...
    def __GetStructMembersInfo(self, TypeInfo):
        ...
        for udtMember in udtTypeData:
            # 1) базовые классы
            if self.__IsBaseClass(udtMember):
                base_off = udtMember.offset // 8
                is_vbase = False
                if hasattr(udtMember, "is_vbaseclass"):
                    try:
                        is_vbase = udtMember.is_vbaseclass()
                    except Exception:
                        pass
                if not is_vbase:
                    # «расплющиваем» поля базы в производный
                    for fm in self.__FlattenBaseMembers(udtMember.type, base_off):
                        members.append(fm)
                else:
                    # для виртуальной базы оставим читаемый плейсхолдер, без случайных "unnamed-…"
                    placeholder = PdbGeneratorPy.MemberData()
                    base_name = udtMember.type.get_type_name() or self.GetTypeName(udtMember.type)
                    placeholder.Name = base_name
                    placeholder.TypeName = base_name
                    placeholder.Offset = base_off
    ...
    def __IsBaseClass(self, m):
        # Совместимо с разными версиями IDA
        try:
            if hasattr(m, "is_baseclass") and m.is_baseclass():
                return True
        except Exception:
            pass
        try:
            bc  = getattr(ida_typeinf, "UDTMF_BASECLASS", 0x01)
            vbc = getattr(ida_typeinf, "UDTMF_VBASECLASS", 0x02)
            return bool(getattr(m, "props", 0) & (bc | vbc))
        except Exception:
            return False

    def __FlattenBaseMembers(self, base_ti, base_off):
        flat = PdbGeneratorPy.MembersData()
        for bm in self.__GetStructMembersInfo(base_ti):
            nm = PdbGeneratorPy.MemberData()
            nm.Name = bm.Name
            nm.TypeName = bm.TypeName
            nm.Offset = base_off + bm.Offset
            nm.Bitfield = bm.Bitfield
            flat.append(nm)
        return flat
    ...
    def GetTypeName(self, TypeInfo):
        # 1) если у типа уже есть имя — используем его
        named = TypeInfo.get_type_name()
        if named:
            return named

        # 2) typedef
        if TypeInfo.is_typeref():
        ...
        if not TypeInfo.is_forward_decl():
            ...
            if simpleType:
                d = TypeInfo.dstr()
                # в IDA wchar_t часто представлен как __int16
                if simpleType.endswith("__int16") and d.endswith(("wchar_t", "WCHAR")):
                    return "wchar_t"
                return simpleType

        # 4) косметика для указателей
        d = TypeInfo.dstr()
        if d == "void *__ptr32":
            return "void *"

        # 5) только реально безымянные UDT разворачиваем и именуем
        if TypeInfo.is_udt() and not TypeInfo.get_type_name():
            return self.__InsertUnnamedStructDataAndGetItsName(TypeInfo)
